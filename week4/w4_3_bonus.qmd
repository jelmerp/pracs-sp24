...
If you use `while` loops, you'll very rarely need arrays (next section),
and conversely, if you like to use arrays, you may not need `while` loops much.

In the example below,
`while read -r` will be true as long as there is a line left to be read from the file, 
...
Alternatively, you can operate on the file content before inputting it into the loop:
...
Finally, we can extract columns directly as follows:
...
The main use of arrays is with loops:
...
but there are scenarios in which this is not the case or you are simply not processing files.
