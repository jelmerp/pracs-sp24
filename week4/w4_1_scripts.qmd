...
subtitle: "Week 4 - Shell scripts - Part I"
pagetitle: "PRACS24: Shell scripting"
...
This week, you will learn how to write shell scripts and then how to use shell scripts
to run programs, like various bioinformatics tools, with command-line interfaces (CLIs).
Next week's material will then cover the next step to get you ready to analyze
large-scale omics datasets at OSC:
submitting your shell scripts as batch compute jobs, and using software at OSC.
...
The quote above uses the word "Bash", and we'll see that come by some more this week.
There are multiple different Unix shells and the specific one we have been using,
which is by far the most common, is the Bash shell.
:::
...
instead of running all your code interactively, line-by-line:

- It is a good way to save and organize your code.
- You can easily rerun scripts and re-use them in similar contexts.
- Related to the point above, they provide a first step towards automating your workflow.
- You can turn having to run many consecutive commands into running a single one,
  namely calling the script.
...
- Run scripts remotely without needing to stay connected to the runnig process,
...
Scripts form the basis for *analysis pipelines/workflows* and if we code things cleverly,
it should be relatively straightforward to rerun much of our project workflow:
...
- Submitting scripts to the Slurm job scheduler (week 5)
- Making software available at OSC (week 5)
...
Such a line starts with **`#!`**, basically marking it as a special type of comment.
...
With these settings, **the script terminates** with an appriate error message if:
...
or in other words, to run the script "as a command".
...
# (Before this, we may have determined the file type based on the file extension,
#  and created the variable $filetype)
```
...
```sh
# '-f' is true if the file exists, and '! -f' is true if the file doesn't exist
...
::: exercise
#### {{< fa user-edit >}} Exercise: Conditionals I

1. Set up an `if` statement that tests whether the script `template.sh` exists and is a regular file.
2. If it is (`then` block), report the outcome with `echo` (e.g. "The file is found").
3. If it is not (`else` block), also report that outcome with `echo` (e.g. "The file is not found").
...
were passed to the script, to our `printname.sh` script.
